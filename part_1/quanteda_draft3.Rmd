...
### Installing **quanteda**
Install the **quanteda** package from [CRAN](https://cran.r-project.org/package=quanteda):

```{r install_quanteda, exercise=TRUE, warning = FALSE}
install.packages("quanteda")
```

Also install the **readtext** package: ** what is readtext??
```{r install_readtext, exercise=TRUE, warning = FALSE}
...
4. Have a look at your tokens using `head()`<br>
...
**Removing stop words example and MCQ**
...
_Exercise: fill in the "..." spaces to choose the keywords you are interested in, remember you can use **stem words** followed by an asterisk (this will include all words derived form that stem)_
...
textplot_xray(
     kwic(tokens(data_corpus_inaugural_subset), pattern = "immig*"),
...
```{r project, exercise=TRUE, exercise.lines = 5}
...
```{r project-solution, exercise=FALSE, exercise.reveal_solution = FALSE}
# Edit docnames
